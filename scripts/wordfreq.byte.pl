...
#   堆叠句 = 加权词数 ≥ max(3, 句字符数/15)
# ★v9.5.7 整书修正：按绝对数量报 ⚠️ 在长文上必然误报（9 部真人整书实测 0-24 句/百万字级），
#     改为报告**每万字堆叠句数密度**——密度才是"堆叠是否成问题"的判据：
#     AI 文（2千字 3 句堆叠）≈ 1500 句/万字；真人整书最高（斗破 24 句/657万字）≈ 0.04 句/万字。
#     脚本只报告密度，不标 ⚠️/❌（是否达标由 05_26 4.1c 按文体/章节判定）。
...
my $stackPer10k = sprintf("%.2f", 10000.0 * $stack / $total);

print "总字数=$total\n";
for my $r (sort { $b->{perK} <=> $a->{perK} } @results) {
    printf "  [%s] %s: %.2f/千字 (%d次)%s\n", $r->{tier}, $r->{word}, $r->{perK}, $r->{n}, $r->{flag};
}
print "连环堆叠句: $stack 句 ($stackPer10k 句/万字)" . ($stack ? " | 示例: $stackSample" : "") . "\n";
